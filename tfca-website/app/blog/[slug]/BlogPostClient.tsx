...
export default function BlogPostClient({ post, related }: { post: Post; related: Post[] }) {
  ...
  const renderContent = (content: string) => {
    return content.split('\n').map((line, i) => {
      ...
      if (trimmed.startsWith('## ')) return <h2 key={i} style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.7rem", fontWeight: 600, color: "#C9A84C", margin: "36px 0 14px" }}>{trimmed.slice(3)}</h2>;
      if (trimmed.startsWith('### ')) return <h3 key={i} style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.3rem", fontWeight: 600, color: "white", margin: "24px 0 10px" }}>{trimmed.slice(4)}</h3>;
      if (trimmed.startsWith('- ')) return <li key={i} style={{ fontSize: "1rem", color: "rgba(255,255,255,0.75)", lineHeight: 1.8, marginBottom: "8px", marginLeft: "20px" }}>{trimmed.slice(2)}</li>;
      if (trimmed.startsWith('|')) return null; // skip table lines
      if (trimmed.match(/^\d\./)) return <li key={i} style={{ fontSize: "1rem", color: "rgba(255,255,255,0.75)", lineHeight: 1.8, marginBottom: "8px", marginLeft: "20px" }}>{trimmed.replace(/^\d\.\s*\*\*(.*?)\*\*/, (_, t) => t).replace(/\*\*(.*?)\*\*/g, '$1')}</li>;
      return <p key={i} style={{ fontSize: "1rem", color: "rgba(255,255,255,0.72)", lineHeight: 1.9, fontWeight: 300, marginBottom: "12px" }}>{trimmed.replace(/\*\*(.*?)\*\*/g, '$1')}</p>;
    });
  };
      ...
      <section style={{ padding: "140px 5% 60px", background: "linear-gradient(135deg, #050D1A 0%, #0D1F3C 100%)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: "800px" }}>
          ...
          <div style={{ display: "flex", gap: "12px", alignItems: "center", marginBottom: "16px", flexWrap: "wrap" }}>
            <span style={{ background: "linear-gradient(135deg, #C9A84C, #A0792E)", color: "#0D1F3C", padding: "4px 14px", borderRadius: "2px", fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>{post.category}</span>
            <span style={{ fontSize: "0.75rem", color: "#8899AA" }}>{post.date}</span>
            <span style={{ fontSize: "0.75rem", color: "#8899AA" }}>• {post.readTime}</span>
          </div>
          <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(2rem, 4vw, 3.2rem)", fontWeight: 600, lineHeight: 1.15 }}>{post.title}</h1>
          <p style={{ fontSize: "1rem", color: "rgba(255,255,255,0.6)", lineHeight: 1.9, marginTop: "20px", fontWeight: 300 }}>{post.excerpt}</p>
        </div>
      </section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: "60px", alignItems: "start" }} className="article-layout">
          ...
          <div>
            ...
            <div style={{ marginTop: "48px", paddingTop: "32px", borderTop: "1px solid rgba(201,168,76,0.15)", display: "flex", alignItems: "center", gap: "16px", flexWrap: "wrap" }}>
              <span style={{ fontSize: "0.72rem", letterSpacing: "0.15em", textTransform: "uppercase", color: "#8899AA" }}>Share this article:</span>
              {["WhatsApp", "LinkedIn", "Twitter"].map((s) => (
                <a key={s} href="#" style={{ fontSize: "0.72rem", color: "#C9A84C", border: "1px solid rgba(201,168,76,0.3)", padding: "6px 14px", borderRadius: "2px", textDecoration: "none" }}>{s}</a>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            <div style={{ background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: "4px", padding: "28px" }}>
              <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.2rem", marginBottom: "12px" }}>Need This Service?</h3>
              <p style={{ fontSize: "0.78rem", color: "#8899AA", lineHeight: 1.7, marginBottom: "20px" }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: "4px", padding: "24px" }}>
              <h4 style={{ fontSize: "0.7rem", letterSpacing: "0.18em", textTransform: "uppercase", color: "#8899AA", marginBottom: "16px" }}>WhatsApp Us</h4>
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                <a href="https://wa.me/2349077504605" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: "8px", background: "#25D366", color: "white", padding: "10px 14px", borderRadius: "4px", textDecoration: "none", fontSize: "0.78rem", fontWeight: 600 }}>
                  💬 +234 907 750 4605
                </a>
                <a href="https://wa.me/2348062250170" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: "8px", background: "#25D366", color: "white", padding: "10px 14px", borderRadius: "4px", textDecoration: "none", fontSize: "0.78rem", fontWeight: 600 }}>
                  💬 +234 806 225 0170
                </a>
              </div>
            </div>
